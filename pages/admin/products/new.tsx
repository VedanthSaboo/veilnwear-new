...
const NewProductPageBase: NextPage = () => {
  const router = useRouter();
  const { firebaseUser } = useAuth();

  const { register, handleSubmit } = useForm<NewProductFormValues>({
    defaultValues: {
      name: '',
      description: '',
      price: 0,
  ...
  const onSubmit = async (values: NewProductFormValues) => {
    if (!firebaseUser) {
      toast.error('You must be logged in as admin to create products.');
      return;
    }
    ...
    try {
      ...
      const stockNum = Math.max(0, Number(values.stock ?? 0) || 0);
      ...
      if (!res.ok) {
        console.error('Create product failed', { status: res.status, body: parsed });
        toast.error(`Create product failed: ${parsed?.message ?? parsed ?? 'Server error'}`);
        return;
      }
      ...
      console.error('Create product error (client):', err);
  ...
  return (
    <>
      <Head>
        <title>New Product | Admin - Veilnwear</title>
      </Head>

      <AdminLayout>
        <div className="max-w-3xl">
          <header className="mb-6">
            <h1 className="text-2xl font-semibold text-neutral-50">New Product</h1>
            <p className="text-sm text-neutral-400">Create a new product. Upload images or paste URLs.</p>
          </header>
...
const NewProductPage = withAdminAuth(NewProductPageBase);
